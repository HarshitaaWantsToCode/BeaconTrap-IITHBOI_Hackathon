...
async def run_startup_migrations():
    ...
    async with engine.begin() as conn:
        await conn.execute(text(
            "ALTER TABLE cases ADD COLUMN IF NOT EXISTS blockchain_tx_hash VARCHAR"
        ))
        await conn.execute(text(
            "ALTER TABLE cases ADD COLUMN IF NOT EXISTS blockchain_block INTEGER"
        ))
        await conn.execute(text(
            "ALTER TABLE cases ADD COLUMN IF NOT EXISTS blockchain_timestamp TIMESTAMP"
        ))
