...
export default function SocPanel({
  ...
  return (
    <div className={`bg-[var(--bg-panel)] border border-[var(--border)] rounded-sm overflow-hidden ${className}`}>
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-[var(--border)] bg-[var(--bg-panel-alt)]">
        <div className="flex items-center gap-2.5">
          ...
          {badge && (
            <span className={`text-[9px] font-mono px-2 py-0.5 rounded-sm border uppercase tracking-wider font-bold ${badgeColor}`}>
              {badge}
            </span>
          )}
        </div>
