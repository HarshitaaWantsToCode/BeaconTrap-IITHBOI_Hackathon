        ...
        <div className="flex items-center gap-4 text-[10px] uppercase">
          {(["critical", "high", "medium", "low"] as const).map((sev) => (
            <div key={sev} className="flex items-center gap-1.5">
              <span className={`w-2.5 h-2.5 rounded-xs border ${SEVERITY_STYLE[sev].bg} ${SEVERITY_STYLE[sev].border}`} />
                  ...
                  {tacticCells.map((cell) => {
                    ...
                    return (
                      <div
                        ...
                        className={`px-3 py-1.5 rounded-sm border ${style.bg} ${style.border} flex items-center justify-between gap-3 min-w-[140px]`}
                      >
                        <div>
                        ...
                        <span className="px-1.5 py-0.5 rounded-sm bg-[var(--bg-base)] border border-[var(--border)] text-[10px] font-bold text-[var(--text-primary)]">
                          {cell.count}
                        </span>
                      </div>
                    );
