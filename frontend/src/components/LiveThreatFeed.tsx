...
export default function LiveThreatFeed() {
  ...
  return (
    <div className="bg-[var(--bg-panel)] border border-[var(--border)] rounded-sm p-4 flex flex-col h-[400px] font-mono">
      {/* Panel Header */}
      ...
      <div className="flex-1 overflow-y-auto space-y-2.5 pr-1 min-h-0 select-none font-mono">
        {feedList.map((alert) => {
          ...
          return (
            <div
              key={alert.id}
              className="p-3 bg-[var(--bg-panel-alt)] border border-[var(--border)] rounded-sm flex flex-col gap-1.5 transition-colors hover:border-[var(--accent)]/40"
            >
              {/* Severity & Timestamp Row */}
              <div className="flex items-center justify-between font-mono text-[9px]">
                <span className={`inline-flex items-center gap-1 font-bold border rounded-sm px-1.5 py-0.5 uppercase ${style.text} ${style.bg}`}>
